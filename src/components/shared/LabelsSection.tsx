...
interface LabelsSectionProps {
  ...
  onRemove?: () => void;
  /** Render inline (for Notion-style property grid) */
  inline?: boolean;
}
...
export function LabelsSection({
  ...
  const renderDropdown = (position: "left-0" | "right-0") => {
    if (!showDropdown) return null;
    return (
      <div className={cn("absolute top-full mt-1 w-72 bg-bg-elevated border border-border rounded-lg shadow-xl z-50 overflow-hidden", position)}>
        {editingLabel ? (
          /* Edit label form */
  ...
  // Inline mode - for Notion-style property grid
  if (inline) {
    return (
      <div className={cn("flex items-center flex-wrap gap-1.5", className)}>
        ...
        ) : assignedLabels.length === 0 && !showDropdown ? (
          <span className="text-xs text-muted-foreground/60 italic">Empty</span>
        ) : (
          assignedLabels.map((label) => (
            <LabelChip key={label.id} label={label} onClick={() => setShowDropdown(true)} onRemove={() => handleRemoveLabel(label)} />
          ))
        )}
        <div className="relative" ref={dropdownRef}>
          <button
            ...
            className="inline-flex items-center justify-center w-5 h-5 rounded text-muted-foreground hover:bg-bg-hover hover:text-text transition-colors"
            title="Add label"
          >
            <Plus className="w-3 h-3" />
          </button>
          {renderDropdown("left-0")}
        </div>
      </div>
    );
  }

  // Default mode - full section with header
  return (
